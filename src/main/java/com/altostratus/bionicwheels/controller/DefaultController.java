...
public class DefaultController {
	...
	public ModelAndView dashboard(HttpServletRequest request, Principal principal) {
		...
		if (request.isUserInRole("ROLE_ADMIN")) {
			...
			logger.info(principal.getName() + " (ROLE_ADMIN) is Entering the dashboard");

			List<BarGraphModel> bgms = new ArrayList<BarGraphModel>();
			List<BarGraphModel2> bgms2 = new ArrayList<BarGraphModel2>();

			if (settings == null || productService.getAllProducts().size() == 0 || categoryService.getAllCategories().size() == 0) {
				...
				if (settings.getFilterBy().equalsIgnoreCase("CATEGORY")) {
					...
					for (Category category : categories) {
						...
						Integer sizeOfAboveNum = productService.getProductsByCategoryAboveNumber(category, aboveNum).size();
						// logger.info(sizeOfAboveNum.toString());
						Integer sizeOfBelowNum = productService.getProductsByCategoryBelowNumber(category, belowNum).size();
						Integer totalSize = productService.getProductsByCategory(category).size();
					...
					logger.info("settings category choice is: " + settings.getCategoryChoice());
					// error starts here
					Category category = categoryService.getCategoryByName(settings.getCategoryChoice());
					...
					for (Product product : products) {
						if (product.getProductTire() == null) {
							...
							bgm2.setCeilingValue(settings.getCeilingValue().toString());
							bgm2.setFloorValue(settings.getFloorValue().toString());

							bgms2.add(bgm2);
						} else if (product.getProductTire() != null) {
							logger.info("product-tire is: " + product.getProductTire().toString());
							BarGraphModel2 bgm2 = new BarGraphModel2();
					...
					for (Brand brand : brands) {
						...
						Integer sizeOfAboveNum = productService.getProductsByBrandAboveNumber(brand, aboveNum).size();
						// logger.info(sizeOfAboveNum.toString());
						Integer sizeOfBelowNum = productService.getProductsByBrandBelowNumber(brand, belowNum).size();
						Integer totalSize = productService.getProductsByBrand(brand).size();
					...
					Brand brand = brandService.getBrandByBrandName(settings.getBrandChoice());
					...
					for (Supplier supplier : suppliers) {
						...
						Integer sizeOfAboveNum = productService.getProductsBySupplierAboveNumber(supplier, aboveNum).size();
						// logger.info(sizeOfAboveNum.toString());
						Integer sizeOfBelowNum = productService.getProductsBySupplierBelowNumber(supplier, belowNum).size();
						Integer totalSize = productService.getProductsBySupplier(supplier).size();
					...
					Supplier supplier = supplierService.getSupplierBySupplierName(settings.getSupplierChoice());
		...
		} else if(request.isUserInRole("SERVICE_MANAGER")){
			logger.info(principal.getName() + " has logged in.");
			ModelAndView mnv = new ModelAndView("redirect:/service-transaction/form");
			return mnv;
		}
		else {
			if (settings == null || productService.getAllProducts().size() == 0 || categoryService.getAllCategories().size() == 0) {
				...
				logger.info(principal.getName() + " has logged in.");
				ModelAndView mnv = new ModelAndView("redirect:/stocktransaction-out");
				return mnv;
			}
		}
	}
	...
	// @RequestMapping(value = { "", "/", "/dashboard", "/favicon.ico" }, method
	// = RequestMethod.GET)
	// public ModelAndView dashboardStaff(HttpServletRequest request) {
	// ModelAndView mnv = new
	// ModelAndView("redirect:/admin/stocktransaction-out");
	// return mnv;
	// }
	...
	//
	// List<ProductInventoryReport> reports = new
	// ArrayList<ProductInventoryReport>();
	// List<ProductInventorySubReport> subReports = new
	// ArrayList<ProductInventorySubReport>();
	// List<Product> products = productService.getAllProducts();
	//
	// ProductInventoryReport productInventoryReport = new
	// ProductInventoryReport();
	//
	// for (Product p : products) {
	//
	// ProductInventorySubReport productInventorySubReport = new
	// ProductInventorySubReport();
	//
